...
"""
from collections import Counter, defaultdict, deque, OrderedDict, ChainMap, UserDict
...
def touch(cache: OrderedDict[str, Product], sku: str, product: Product, capacity: int) -> None:
    """Insert or refresh sku as most-recently-used; evict the LRU entry past capacity."""
    cache[sku] = product
...
def effective_stock(default_levels: dict[str, int], *overrides: dict[str, int]) -> ChainMap:
    """Layer overrides over defaults so a sku resolves to the first layer that has it."""
    return ChainMap(*overrides, default_levels)


def merge_catalogs(*catalogs: list[Product]) -> list[Product]:
    """Flatten any number of catalogs into one list, in order."""
    return [product for catalog in catalogs for product in catalog]


def build_product(**fields: object) -> Product:
    """Construct a record from a fields dict, spread in as keyword arguments (**)."""
    return ProductRecord(**fields)


class Catalog(UserDict):
    """A SKU-keyed catalog that validates products as they are inserted."""

    def __setitem__(self, sku: str, product: Product) -> None:
        if not sku:
            raise ValueError("sku must be non-empty")
        if product.price_cents < 0 or product.qty < 0:
            raise ValueError("price and qty must be non-negative")
        super().__setitem__(sku, product)


class StockLedger(dict):
    """A SKU -> on-hand-quantity map whose missing keys default to 0.

    Reading an unknown SKU INSERTS a 0 balance and returns it, so
    ``ledger[sku] += delta`` works for a first-time SKU. This is exactly the
    hook collections.defaultdict uses internally.
    """
