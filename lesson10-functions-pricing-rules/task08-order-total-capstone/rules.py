...
_pricing_calls = 0  # module-level counter, bumped via global below


def make_running_total():
    """Return an add(n) function that keeps a running total across its own calls.

    The inner add() needs to REASSIGN `total` in the enclosing function. Without a
    declaration, assigning `total` inside add() would just create a new local. The
    `nonlocal` keyword says 'this name lives in the enclosing scope'.
    """
    total = 0
...
def record_pricing_call() -> int:
    """Bump and return the module-level pricing-call counter.

    `global` is the module-scope equivalent of nonlocal: it lets this function
    REASSIGN a name defined at module level instead of shadowing it with a local.
    """
    global _pricing_calls
    _pricing_calls += 1
    return _pricing_calls


def expand_bundle(catalog: dict, sku: str, qty: int, _depth: int = 0) -> list[tuple[str, int]]:
    """Recursively explode a bundle SKU into flat (leaf_sku, qty) lines.

    `catalog` maps a SKU to its components -- a list of (component_sku, component_qty)
    for a bundle, or nothing for a plain product (a leaf). The function calls ITSELF
    on each component, multiplying quantities as it descends. The base case (a leaf)
    stops the recursion. A self-referencing bundle ('kit contains kit') would recurse
    forever, so the function caps its depth at a fraction of Python's recursion ceiling
    (sys.getrecursionlimit()) and raises early with a clear message, well before the
    interpreter would hit its own limit with a generic RecursionError.
    """
    if _depth > sys.getrecursionlimit() // 10:
    ...
    for component_sku, component_qty in components:
        leaves += expand_bundle(catalog, component_sku, component_qty * qty, _depth + 1)
    return leaves


def order_total(lines: list[tuple[str, int, int]], *, tier_percent: int = 0,
                coupons: tuple[int, ...] = (), tax_rate_bps: int = 0) -> dict:
    """Compute a fully itemized order total in cents from priced (sku, price, qty) lines.

    Composes the phase's functions: subtotal the lines, take the customer-tier
    discount, stack each coupon, then apply keyword-only tax. The discount/tax
    options are keyword-only so a call reads as order_total(lines, tax_rate_bps=825).
    (Bundle lines are flattened with expand_bundle before being priced into `lines`.)
    """
    subtotal = sum(price_cents * qty for _, price_cents, qty in lines)
    after_tier = apply_percent(subtotal, tier_percent)
    after_coupons = stack_discounts(after_tier, *coupons)
    tax = apply_tax(after_coupons, rate_bps=tax_rate_bps)
    return {
        "subtotal": subtotal,
        "after_discounts": after_coupons,
        "tax": tax,
        "total": after_coupons + tax,
    }
