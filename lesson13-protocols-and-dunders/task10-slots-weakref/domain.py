...
class Product:
    ...
    # Upgraded from a plain dict: weak values let a Product that is dropped
    # everywhere else be garbage-collected instead of leaking here forever.
    _instances: "weakref.WeakValueDictionary[str, Product]" = weakref.WeakValueDictionary()
    ...
    # "__weakref__" is added so a slotted Product can still be weakly referenced (the
    # registry below needs that); without it weakref.ref(product) raises TypeError.
...
class LineRef:
    """A 1-based invoice line number that behaves like an integer index.

    By implementing __index__ (NOT __int__), a LineRef can be used directly as a list
    index, inside a slice, and as a range() argument -- always as an exact int. Using
    __index__ rather than __int__ means there is no lossy float-style conversion: a
    line position is always a whole number."""
    ...
    def __index__(self) -> int:
        """The canonical 'give me a real int, losslessly' hook (operator.index uses it)."""
        return self.number

    def __repr__(self) -> str:
        return f"LineRef({self.number})"


class ProductRegistry:
    """A SKU -> Product identity map backed by weak references.

    canonical(product) returns the one shared instance for a SKU. Because the map holds
    only WEAK references, a Product dropped everywhere else is evicted automatically, so
    the registry tracks the working set instead of leaking every Product ever seen."""
